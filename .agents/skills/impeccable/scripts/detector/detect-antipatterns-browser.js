...
if (IS_BROWSER) {
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      void('%c[impeccable] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
    ...
    for (const { el, findings } of allFindings) {
      for (const f of findings) {
        void(`%c${f.type || f.id}%c ${f.detail || f.snippet}`,
  ...
  function reportVisualContrastError(err, detail = {}) {
    ...
    if (EXTENSION_MODE) {
      ...
      void('[impeccable] visual contrast scan failed', err);
    }
  }
    ...
    if (window.__IMPECCABLE_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          ...
          void('[impeccable] scan failed', err);
        }
      };
