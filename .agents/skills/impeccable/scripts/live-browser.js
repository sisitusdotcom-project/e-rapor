...
(function () {
  ...
  if (!sessionState) {
    void('[impeccable] live-browser-session.js was not loaded. Live mode cannot start safely.');
  ...
  if (!domHelpers) {
    void('[impeccable] live-browser-dom.js was not loaded. Live mode cannot start safely.');
    window.__IMPECCABLE_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      void('[impeccable] Refusing to render empty variant cycling state:', reason);
  ...
  function parseVariantParams(variantEl) {
    ...
    try {
      ...
      void('[impeccable] Invalid data-impeccable-params JSON:', err.message);
      return [];
    }
  }
  ...
  async function applyEditing() {
    ...
    try {
      ...
      void('[impeccable] manual edit stash failed:', err);
  ...
  async function fetchPendingCount() {
    try {
      ...
      void('[impeccable] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        void('[impeccable] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          void('[impeccable] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      void('[impeccable] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      void('[impeccable] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      void('[impeccable] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      void('[impeccable] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      void('[impeccable] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      void('[impeccable] Failed to mount Svelte variant ' + variantNum + ' for ' + sessionId + ':', err);
      return false;
    }
  }
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      if (!liveEl?.parentElement) {
        void('[impeccable] Could not find original element in live DOM.');
        arrivedVariants = Number(manifest.count) || expectedVariants || 1;
        expectedVariants = arrivedVariants;
      ...
      positionBar();
      saveSession();
      void('[impeccable] Mounted ' + arrivedVariants + ' Svelte component variants.');
    } catch (err) {
      void('[impeccable] Failed to mount Svelte component variants:', err);
  ...
  function abortSvelteComponentInjection(sessionId, message) {
    try {
      ...
      void('[impeccable] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        if (!srcWrapper) {
          void('[impeccable] Variant wrapper not found in source file.');
          return;
        }
        ...
        positionBar();
        saveSession();
        void('[impeccable] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        void('[impeccable] Failed to fetch source:', err);
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          void('[impeccable] Live mode connected.');
        ...
        case 'error':
          ...
          if (maybeCompleteSteer(msg)) break;
          void('[impeccable] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        void('[impeccable] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      void('[impeccable] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        void('[impeccable] Failed to send event:', err);
        throw err;
      }
      void('[impeccable] Dropped optional live event:', err);
      return null;
    }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          void('[impeccable] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      void('[impeccable] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          void('[impeccable] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        void('[impeccable] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      void('[impeccable] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      void('[impeccable] shader bitmap decode failed:', err);
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            void('[impeccable] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      void('[impeccable.voice] recognition error:', code);
    ...
    try {
      ...
      void('[impeccable.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function teardown() {
    ...
    window.__IMPECCABLE_LIVE_INIT__ = false;
    void('[impeccable] Live mode exited.');
  }
  ...
  function init() {
    ...
    if (!resumeSession()) {
      void('[impeccable] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          void('[impeccable] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      void('[impeccable] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
